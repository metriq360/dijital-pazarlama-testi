...
export const handler = async (event) => {
    ...
    try {
        ...
        const advicePromise = openai.chat.completions.create({
            model: "gpt-3.5-turbo",
            messages: [{ role: "user", content: `Bir kullanıcı, dijital pazarlama testinden 100 üzerinden ${Math.round(percentage)} puan aldı. Bu, '${performanceLevel}' bir skordur. Bu kullanıcıya, skorunu dikkate alarak, tek cümlelik, motive edici ve aksiyona yönelik bir tavsiye ver. METRIQ360'ın bir hizmetine veya IQ360 yaklaşımına atıfta bulunarak onlarla iletişime geçmeye teşvik et.` }],
            max_tokens: 150,
            temperature: 0.8,
        });
        ...
        const msgToUser = { to: userInfo.email, from: 'iletisim@metriq360.com', subject: `🚀 Dijital Pazarlama Raporunuz, ${userInfo.name}!`, html: `<h2>Merhaba ${userInfo.name},</h2><p>Testi tamamladığınız için teşekkürler! Aşağıda raporunuzu bulabilirsiniz:</p><hr>${reportHtml}` };
        const msgToAdmin = { to: 'bilgi@metriq360.com', from: 'iletisim@metriq360.com', subject: `Yeni Test Tamamlandı: ${userInfo.name} ${userInfo.surname}`, html: `<h2>Yeni test sonucu:</h2><p><strong>Kullanıcı:</strong> ${userInfo.name} ${userInfo.surname}</p><p><strong>E-posta:</strong> ${userInfo.email}</p><p><strong>Sektör:</strong> ${userInfo.sector}</p><p><strong>Puan:</strong> ${totalScore} / ${totalMaxScore}</p><hr><h3>Oluşturulan Rapor:</h3>${reportHtml}` };
